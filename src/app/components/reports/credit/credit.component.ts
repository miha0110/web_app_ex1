  ...
  async generate() {
    ...
    this.nullifyUsage();
     ...
     for (var i: number = 0; i < this.systems.length; i++) {
       ...
       if (parseInt(this.systems[i].creditpulse) > 0) {
       this.totalPulses += parseInt(this.systems[i].creditpulse);
        }
      }  
    }
...
disableSelect(){
      ...
      .subscribe((res) => {
         console.log(res);
          this.systems.forEach(s=>{
           if(s.distid===this.chosenDist)
           {
            this.usage1 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulse']) - Number(res[0][String(s.serialNumber)]['hppulse']));
            if(res[1][String(s.serialNumber)]['hppulseright']){
              this.usage1 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulseright']) - Number(res[0][String(s.serialNumber)]['hppulseright']));
            }
           }
          })
    ...
    store2.find(query3)
      .subscribe((res) => {
          ...
          this.systems.forEach(s=>{
            if(s.distid===this.chosenDist)
            {
            this.usage2 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulse']) - Number(res[0][String(s.serialNumber)]['hppulse']));
            if(res[1][String(s.serialNumber)]['hppulseright']){
              this.usage2 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulseright']) - Number(res[0][String(s.serialNumber)]['hppulseright']));
            }
            }
          })
    ...
    store2.find(query5)
      .subscribe((res) => {
          ...
          this.systems.forEach(s=>{
            if(s.distid===this.chosenDist)
            {
            this.usage3 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulse']) - Number(res[0][String(s.serialNumber)]['hppulse']));
            if(res[1][String(s.serialNumber)]['hppulseright']){
              this.usage3 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulseright']) - Number(res[0][String(s.serialNumber)]['hppulseright']));
            }
            }
          })
    ...
    store2.find(query7)
      .subscribe((res) => {
          ...
          this.systems.forEach(s=>{
            if(s.distid===this.chosenDist)
            {
            this.usage4 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulse']) - Number(res[0][String(s.serialNumber)]['hppulse']));
            if(res[1][String(s.serialNumber)]['hppulseright']){
              this.usage4 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulseright']) - Number(res[0][String(s.serialNumber)]['hppulseright']));
            }
            }
          })
    ...
    store2.find(query9)
      .subscribe((res) => {
          ...
          this.systems.forEach(s=>{
           if(s.distid===this.chosenDist)
           {
            this.usage5 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulse']) - Number(res[0][String(s.serialNumber)]['hppulse']));
            if(res[1][String(s.serialNumber)]['hppulseright']){
              this.usage5 += Math.abs(Number(res[1][String(s.serialNumber)]['hppulseright']) - Number(res[0][String(s.serialNumber)]['hppulseright']));
            }
           }
          })
  ...
  purchase(){
    Kinvey.CustomEndpoint.execute('DDDgetCodes').then((codes: Code[]) => {
      codes.forEach(element => {
               if(this.isDistributor(element.SystemSn)){
  ...
  isDistributor(sn: string){
      var i;
      for(i = 0; i < this.systems.length && String(this.systems[i].serialNumber) != String(sn)  ; i++){}
    
      if(i < this.systems.length && this.systems[i].distid == this.chosenDist)
      {
        return true;
      }
      else{
